...
namespace Scene
{
    ...
    struct Entry
    {
        ...
        void (*loop)(const Data &) = nullptr;    /// Will called repeatedly until endTime has been reached.
        void (*cleanup)(const Data &) = nullptr; /// Clean up all your data / gfx etc. here when the scene ends.
    } __attribute__((aligned(4), packed));

    /// @brief Starts play a sequence of scenes.
    void play(const Entry *entries, uint32_t nrOfEntries, uint32_t startIndex = 0, bool loopScene = false);
