...
namespace Adpcm
{
    ...
    extern "C" auto UnCompWrite32bit_8bit_ASM(const void *data, uint32_t dataSize, uint8_t *dst[2]) -> void;
    ...
    extern "C" auto UnCompGetSize_8bit_ASM(const void *data) -> uint32_t;
    ...
    /// @param data Pointer to 4-bit ADPCM data
    /// @param dst Pointers to output sample buffer(s). Will be written to as bytes
    auto UnCompWrite32bit_8bit(const void *data, uint32_t dataSize, uint8_t *dst[2]) -> uint32_t;

    /// @brief Get stored uncompressed size of sample data after decoding
    /// @param data Pointer to ADPCM data
    auto UnCompGetSize_8bit(const void *data) -> uint32_t;
}
