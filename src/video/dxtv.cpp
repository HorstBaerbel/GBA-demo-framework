...
namespace Video
{
    ...
    IWRAM_FUNC void DXTVUnCompWrite16bit(const uint32_t *data, uint32_t *dst, const uint32_t *prevSrc, const uint32_t prevSrcLineStride, uint32_t width, uint32_t height)
    {
    ...
    uint32_t DXTVUnCompGetSize(const uint32_t *data)
    {
        const Video::DxtvFrameHeader frameHeader = Video::DxtvFrameHeader::read(data);
