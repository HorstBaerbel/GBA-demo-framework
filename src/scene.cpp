...
void play(const Entry *entries, uint32_t nrOfEntries, uint32_t startIndex)
{
    ...
    for (uint32_t i = startIndex; i < nrOfEntries; ++i)
    {
        ...
        // set event handler to our routine
        Sound::setSongEventHandler(entry.playerEvent);
        // set up start / end time
        const Math::fp1616_t startTime = Time::getTime();
        ...
        if (entry.loop != nullptr)
        {
            ...
            printf("Running loop");
#endif
            while (Time::getTime() < endTime || entry.duration <= 0)
            {
#if defined(KEYSDOWN_IN_DATA) || defined(SCENE_CONTROL)
                scanKeys();
...
#endif
#endif
                sceneData.t = Math::fp1616_t::ONE - (endTime - Time::getTime()) / sceneData.duration;
