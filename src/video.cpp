...
void getBackgroundPalette(uint16_t *palette, uint32_t startIndex, uint32_t count)
{
    startIndex = startIndex > 255 ? 255 : startIndex;
    count = startIndex + count > 256 ? 256 - startIndex : count;
    Memory::memcpy16(palette, &BG_PALETTE[startIndex], count);
}

void getBackgroundPalette4(Palette::Palette16 &palette, uint32_t paletteIndex, uint32_t count)
{
    paletteIndex = paletteIndex > 15 ? 15 : paletteIndex;
    count = count > 16 ? 16 : count;
    getBackgroundPalette(static_cast<uint16_t *>(palette), 16 * paletteIndex, count);
}

void getBackgroundPalette8(Palette::Palette256 &palette, uint32_t count)
{
    getBackgroundPalette(static_cast<uint16_t *>(palette), 0, count);
}

void setBackgroundPalette(const uint16_t *palette, uint32_t startIndex, uint32_t count)
{
    startIndex = startIndex > 255 ? 255 : startIndex;
    count = startIndex + count > 256 ? 256 - startIndex : count;
    Memory::memcpy16(&BG_PALETTE[startIndex], palette, count);
}

void setBackgroundPalette4(const Palette::Palette16 &palette, uint32_t paletteIndex, uint32_t count)
{
    paletteIndex = paletteIndex > 15 ? 15 : paletteIndex;
    count = count > 16 ? 16 : count;
    setBackgroundPalette(static_cast<const uint16_t *>(palette), 16 * paletteIndex, count);
}

void setBackgroundPalette8(const Palette::Palette256 &palette, uint32_t count)
{
    setBackgroundPalette(static_cast<const uint16_t *>(palette), 0, count);
}
